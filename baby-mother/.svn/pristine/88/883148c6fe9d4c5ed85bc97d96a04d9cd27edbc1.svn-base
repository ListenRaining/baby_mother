...
Page({
    ...
    //声音的游戏--赠送
    doShare:function(e){
        if(e.currentTarget.dataset.share_id == 1){
            wx.navigateTo({
                url: '/pages/my/send/send_detail?share_id='+e.currentTarget.dataset.share_id+'&share_type='+e.currentTarget.dataset.share_type+'&share_name='+e.currentTarget.dataset.share_name+'&share_image='+e.currentTarget.dataset.share_image+'&oid='+this.data.userinfo.id+'&send=0'
            })
        }else{
            this.setData({
                isShow:true, //显示弹层
    ...
    onShareAppMessage: function (res) {
        ...
        if(this.data.share_type == 1){
            //产品
            var url = '/pages/product/introduce?id='+this.data.share_id+'&puid='+this.data.uid;
        }else{
            //活动
            var url = '/pages/activity/activityinfo/activityinfo?act_id='+this.data.share_id+'&puid='+this.data.uid;
        }
